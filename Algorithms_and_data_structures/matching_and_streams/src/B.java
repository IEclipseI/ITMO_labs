    ...
    private static class StreamFinder {
        ...
        private void findStream() throws IOException {
            ...
            for (int i = 0; i < m; i++) {
                ...
                g.get(to - 1).edges.add(new Edge(to - 1, from - 1, 0, 0, i + m, cap));
                g.get(to - 1).edges.add(new Edge(to - 1, from - 1, cap, i + 2 * m));
                g.get(from - 1).edges.add(new Edge(from - 1, to - 1, 0, 0, i + 3 * m, cap));
            }
            ...
            List<Edge> ed = cut();
            ed = ed.stream().filter(t -> t.id % (2 * m) < m).sorted(Comparator.comparingInt(t -> t.id % (m * 2))).collect(Collectors.toList());
            System.out.println(ed.size() + " " + maxFlow);
            for (Edge e : ed) {
                System.out.print(e.id % (2 * m) + 1 + " ");
            }
//            List<Edge> sorted = g.stream().map(t -> t.edges).flatMap(List::stream).filter(t -> t.id < m && Math.abs(t.flow) == Math.abs(t.cap))
//                    .sorted(Comparator.comparingInt(t -> t.id)).collect(Collectors.toList());
//            System.out.println(sorted.size() + " " + maxFlow);
//            sorted.forEach(t -> System.out.print(t.id + 1 + " "));
            System.out.println();

        }

        List<Edge> cut() {
            Set<Edge> objects = new HashSet<>();
            for (Vert v : g) {
                for (Edge e :
                        v.edges) {
                    if (dist.get(v.id) != INF && dist.get(e.to) == INF)
                        objects.add(e);
                }
            }
