...
struct solve {
    ...
    solve(ll n, ifstream &&fin) : n(n), fin(std::move(fin)) {
        s = 2 * n;
        t = 2 * n + 1;
        size = 2 * n + 2;
        dist = vt<ll>(size, INF);
    ...
    pair<ll, vt<pair<ll, ll>>> find_min_cost_flow() {
        ll answer = 0;
        ...
        vt<pair<ll, ll>> res;
        for (auto& p : redge) {
            if (p.first->id < 2 * n * n && p.first->flow == 1) {
                res.push_back({p.first->from + 1, p.first->to - n + 1});
            }
        }
        return {answer, res};
    }
...
int main() {
    ...
    fin >> n;
    vt<vert> G;
    solve sol(n, move(fin));
    sol.read_graph();
    auto res = sol.find_min_cost_flow();
    fout << res.first << '\n';
