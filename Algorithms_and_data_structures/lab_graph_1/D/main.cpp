...
//        it++;
//    }
    cout << set_edge_component_color(graph) << '\n';
    for (vertex& v :graph) {
        cout << v.color << ' ';
    }
    return 0;
...
void set_edge_component_color_impl(vertex &v, vec<vertex> &graph, ui color, unordered_set<edge, edge_hash, edge_equals> &bridges) {
    if (v.visited) return;
    v.visited = true;
    v.color = color;
    for(edge &e : v.edges) {
