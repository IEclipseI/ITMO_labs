...
struct aho_corasick {
    ...
    map<ll, ll> sift_string(string s) {
        ...
        while (i < s.size()) {
            ...
            nodes[cur].time = nodes[cur].time == -1? i : nodes[cur].time;
            i++;
        }
        deque<ll> left{0};
        deque<ll> to_do;
...
private:
    void sum(ll cur) {
        if (nodes[cur].time != -1)
            if (nodes[nodes[cur].link].time == -1)
                nodes[nodes[cur].link].time = nodes[cur].time;
            else
                nodes[nodes[cur].link].time = min(nodes[cur].time, nodes[nodes[cur].link].time);
...
int main() {
    ...
    cin.tie(nullptr);
    ifstream fin("search6.in");
    ofstream fout("search6.out");
    int n;
    fin >> n;
    aho_corasick tree;
    aho_corasick reverse_tree;
    vec<string> inp;
    for (int i = 0; i < n; ++i) {
        string tmp;
        fin >> tmp;
        ...
        reverse_tree.add(tmp);
    }
    tree.build_suf_links();
    reverse_tree.build_suf_links();
    string t;
    fin >> t;
    auto res1 = tree.sift_string(t);
    reverse(t.begin(), t.end());
    auto res2 = reverse_tree.sift_string(t);
    for (int i = 0; i < n; ++i) {
        if (res1.find(i)->second >= 0){
            fout << res1.find(i)->second - inp[i].length() + 1 << " ";
        } else {
            fout << res1.find(i)->second << " ";
        }
        if (res2.find(i)->second >= 0)
            fout << t.length() - res2.find(i)->second - 1 << "\n";
        else
            fout << res2.find(i)->second << "\n";
